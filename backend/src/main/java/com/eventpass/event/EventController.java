...
public class EventController {
  ...
  Page<EventResponse> list(
    ...
    return service.search(category, city, startDate, endDate, pageable);
  }
  ...
  ResponseEntity<EventResponse> create(
      @Valid @RequestBody EventRequest r, @AuthenticationPrincipal User u) {
    return ResponseEntity.status(HttpStatus.CREATED).body(service.create(r, u));
  }

  @PutMapping("/{id}")
  @PreAuthorize("hasAnyRole('ORGANIZER','ADMIN')")
  EventResponse update(
      @PathVariable UUID id, @Valid @RequestBody EventRequest r, @AuthenticationPrincipal User u) {
    return service.update(id, r, u);
  }
