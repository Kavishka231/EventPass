...
- Messaging: stable versioned envelopes (`eventId`, `eventType`, `version`, `timestamp`, `aggregateId`, `payload`), transactional outbox persistence, PostgreSQL `FOR UPDATE SKIP LOCKED` claiming, capped exponential backoff, explicit primary/dead-letter topic provisioning, idempotent producers, read-committed consumers, and idempotent consumption.
