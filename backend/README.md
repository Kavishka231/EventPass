...
- Messaging: transactional domain outbox, PostgreSQL `FOR UPDATE SKIP LOCKED` claiming, persisted capped exponential backoff, explicit failed delivery state, admin-triggered recovery, and an idempotent notification consumer backed by `processed_events`.
