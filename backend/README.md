...
- Messaging: stable versioned envelopes (`eventId`, `eventType`, `version`, `timestamp`, `aggregateId`, `payload`), transactional outbox persistence, PostgreSQL `FOR UPDATE SKIP LOCKED` claiming, capped exponential backoff, failed-state recovery, and idempotent consumption.
