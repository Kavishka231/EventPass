...
- Management: protected administrator user/role lifecycle and statistics, venue/seat definitions, organizer-owned draft events, event-specific priced inventory, immutable published schedules/venues, and a locked, resumable cancellation workflow that blocks new sales, immediately invalidates every event ticket, records outbox events, durably refunds confirmed bookings, and releases inventory after successful refunds. An incomplete refund reconciliation is returned explicitly as `EVENT_CANCELLATION_INCOMPLETE` and can be retried safely.
...
- Payments: pending charge and refund attempts are committed before provider calls; success, decline, provider references, timestamps, failure details, and ambiguous provider or database-finalization outcomes requiring reconciliation are durable. Refund transitions are guarded, and a pessimistic booking lock serializes concurrent cancellation requests before refund creation. The provider abstraction serializes and replays same-key financial operations, rejects changed details, and forwards each key through provider adapters. Expiration workers claim bounded batches with PostgreSQL `FOR UPDATE SKIP LOCKED`; cancellation/resale remains supported.
...
- Verification: Spotless and Maven verification pass; Testcontainers covers the HTTP authorization matrix, seat/financial/admin races, competing expiration workers and outbox publishers, duplicate event delivery, resumable cancellation, and the complete active-event cancellation orchestration through refunded payments, invalid tickets, released inventory, and outbox events.
