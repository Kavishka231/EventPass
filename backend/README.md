...
- Authentication: BCrypt passwords, strictly issuer/audience/type-scoped HS256 memory-oriented access tokens, scoped `HttpOnly` refresh cookies, rotating hashed refresh-token families with reuse detection and family revocation, CSRF-protected refresh/logout, logout cookie clearing, active-account enforcement, customer registration, and optional first-admin bootstrap.
...
- Query efficiency: customer booking lists and ticket lists use direct enriched projections, while owned booking details project event, venue, seat, payment, and refund context in one query. Customer pages no longer depend on public event/inventory lookups, and composite PostgreSQL indexes support ownership filters and newest-first ordering.
- Verification: Spotless and Maven verification pass; PostgreSQL/Redis/Kafka Testcontainers covers the HTTP authorization matrix, seat/financial/admin races, competing outbox publishers, the real outbox-to-consumer notification pipeline, duplicate event delivery, retry/DLQ recovery, and complete active-event cancellation orchestration through refunded payments, invalid tickets, released inventory, and outbox events.
- HTTP contracts: MockMvc integration coverage verifies success statuses and payloads, authenticated validation, organizer event ownership, administrator booking access, authorization failures, pagination bounds, idempotency conflicts, and every field in the standard error envelope.
- Authentication tests: unit and PostgreSQL-backed MockMvc flows cover registration, duplicate emails, login cookie attributes, absence of refresh secrets from JSON, inactive accounts, expiry, CSRF enforcement, refresh rotation, logout cookie clearing, and refresh-token replay revocation.
