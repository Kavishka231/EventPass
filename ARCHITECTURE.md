...
The backend is a modular monolith organized by business capability (`auth`, `user`, `event`, `venue`, `seat`, `booking`, `payment`, `ticket`, `notification`, and `common`). Controllers translate HTTP requests, services own state transitions and transaction boundaries, and repositories isolate persistence. The React frontend mirrors these capabilities through typed API services, TanStack Query server state, role-guarded customer/organizer/admin workspaces, and reusable UI primitives; it never replaces server-side authorization or pricing decisions.
