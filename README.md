...
- administrator user/role/statistics and filtered booking-management APIs with self-change and last-active-admin protection, venue management, physical seat creation with capacity checks, organizer-owned events and booking reports, guarded publication/cancellation with immediate ticket invalidation, organizer-owned/admin QR ticket validation and concurrency-safe redemption, automatic booking refunds and inventory release, and per-event pricing/blocking;
...
- PostgreSQL/Redis/Kafka Testcontainers coverage for authorization, suspended/invalid sessions, seat contention, financial/admin races, competing outbox publishers, the complete outbox-to-notification pipeline, duplicate event delivery, consumer retry/DLQ recovery, and the full active-event cancellation chain.
