...
- versioned event envelopes, transactional outbox events with PostgreSQL `SKIP LOCKED` claiming, persisted exponential retries, explicitly provisioned Kafka topics, bounded consumer retries with dead-letter topics, idempotent notification consumption, and durable customer notifications with delivery state and authenticated read APIs;
