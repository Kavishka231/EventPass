...
- Redis TTL locks plus PostgreSQL row and advisory locks, optimistic versions, server-side pricing, user-scoped request-bound booking idempotency, durable payment attempts with reconciliation flags, refunds, cancellation, seat resale, expiration, and cryptographically secure tickets;
