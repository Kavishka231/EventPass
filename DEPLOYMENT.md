# Deployment

Local Compose exposes Kafka to host tools at `localhost:9092` and advertises a separate container-network listener at `kafka:29092`. The application profile uses the internal listener and waits for broker health before startup.

The backend multi-stage Dockerfile at `backend/Dockerfile` builds on JDK 21 and runs on a non-root JRE 21 Alpine user with a liveness health check. Root-level `compose.yml` provides PostgreSQL, Redis, Apache Kafka, and an optional application profile using `backend/` as its build context.

The frontend is a Vite production bundle produced by `npm run build` in `frontend/`. Deploy the generated `dist/` directory through a static host or web server configured to fall back to `index.html` for client-side routes, and set `VITE_API_BASE_URL` to the public `/api/v1` backend base URL at build time.

Set `DATABASE_URL`, `DATABASE_USERNAME`, `DATABASE_PASSWORD`, `REDIS_HOST`, `REDIS_PORT`, `KAFKA_BOOTSTRAP_SERVERS`, and a cryptographically random `JWT_SECRET` of at least 32 bytes. Set stable deployment-specific `JWT_ISSUER`, `JWT_AUDIENCE`, and `JWT_SIGNING_KEY_ID` values; changing any of them invalidates existing access tokens. Readiness includes dependency indicators; liveness uses only application process state. Persist PostgreSQL independently and terminate HTTP TLS at the ingress/load balancer.

The `prod` profile fails startup unless Redis has `REDIS_PASSWORD` authentication and TLS enabled, and Kafka uses `SASL_SSL` with `KAFKA_SASL_MECHANISM` plus a secret-injected `KAFKA_SASL_JAAS_CONFIG`. `REDIS_USERNAME` defaults to the Redis ACL `default` user. The JVM trust store must trust the Redis and Kafka server certificate chains; mount a deployment-specific trust store and configure JVM trust-store options when private certificate authorities are used. Never commit JAAS credentials or trust-store passwords. The custom Kafka readiness indicator reuses these application security properties instead of opening a separate plaintext connection.

Redis connect/command/shutdown timeouts and Kafka delivery/request/retry/reconnect/setup bounds have conservative defaults and corresponding environment overrides in `backend/.env.example`. Tune them against measured network latency and broker failover objectives; keep the Kafka delivery timeout greater than its request timeout. Local Compose deliberately remains unauthenticated plaintext infrastructure and must not be exposed or used as the production deployment topology.

Production requires `CORS_ALLOWED_ORIGINS` as a comma-separated list of exact trusted browser origins. Wildcards are rejected because credentialed requests are enabled. Swagger UI and OpenAPI JSON are disabled in the production profile; expose them only through a separately secured non-production environment. Only health probes are anonymous, while other exposed Actuator endpoints require an administrator.

The application provisions its domain and dead-letter topics by default. Production defaults to three replicas; ensure the Kafka cluster has at least three brokers, or deliberately set `KAFKA_TOPIC_REPLICATION_FACTOR` for a smaller environment. Tune `KAFKA_TOPIC_PARTITIONS`, `KAFKA_TOPIC_RETENTION_MS`, `KAFKA_DLT_RETENTION_MS`, `KAFKA_CONSUMER_GROUP`, and `KAFKA_CONSUMER_CONCURRENCY` for workload and recovery requirements. Automatic broker topic creation is disabled so missing or invalid production topology fails startup visibly.

Optional `BOOTSTRAP_ADMIN_EMAIL` and `BOOTSTRAP_ADMIN_PASSWORD` values create the initial administrator only when the email does not exist; remove them afterward. The production profile has no JWT fallback. Outbox publication can be disabled temporarily with `OUTBOX_ENABLED=false`, but normal production operation requires Kafka and the default enabled publisher.

## PostgreSQL backup and recovery

Use the versioned scripts and full operator runbook in [`ops/postgres/README.md`](ops/postgres/README.md). Backups use PostgreSQL custom format, atomic publication, SHA-256 verification, and prefix-scoped retention. Recovery verification restores into a disposable database and validates the EventPass schema and Flyway history. Restore requires an exact target-database confirmation and runs transactionally. Schedule backups and independent restore verification according to documented recovery-point and recovery-time objectives, retain encrypted off-site copies, and alert on failed or stale backups.
